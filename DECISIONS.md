...
**Status:** Ratified; v0.1 contact queue and Slice A1 source ingestion implemented
...
reuses the current private operator shell, Postgres/Prisma infrastructure, and evidence discipline.
Name commercial records with the `Oi` domain prefix. `OiOpportunity` owns possible funded work and
does not require a known person. `OiPursuit` owns a person-specific commercial motion, contact path,
and relationship state. Keep `AssetOpportunity` content-only. Domain logic lives under
`src/lib/opportunity-intelligence`.

TIF is a deterministic, content-specific template engine today. It is not a general AI or outreach
service. An OIE-to-TIF outreach adapter is possible future work, not implemented reuse and not part
of Slice A1.
...
**Implementation update (2026-07-30):** The v0.1 person-first queue is committed and remains intact.
Slice A1 adds immutable `OiSource` snapshots, person-free `OiOpportunity` records, repeated
provenanced facts, research-gap lifecycle, immutable opportunity-fit scores, the typed `todd-v1`
capability profile, deterministic extraction with verified offsets, and transactional pasted-source
ingestion. The A1 migration is applied. UI, AI, URL fetching, Signals, Initiatives, person linkage,
outreach, and ATS adapters remain deferred. Canonical requirements and current implementation
status:
